...
def _decode_image(image_bytes: bytes) -> tuple[np.ndarray, np.ndarray] | None:
    try:
        ...
        exif_transposed = (
            transposed_image.size != original_size
            or transposed_image.mode != original_mode
        )
        rgb_pil = transposed_image.convert("RGB")
        rgb_converted = rgb_pil.mode != transposed_image.mode
        rgb_pil.load()
    except (UnidentifiedImageError, OSError, ValueError) as exc:
        _trace(f"PIL decode failed exception={type(exc).__name__}: {exc}")
        return None

    rgb_image = np.ascontiguousarray(np.asarray(rgb_pil))
    bgr_image = np.ascontiguousarray(cv2.cvtColor(rgb_image, cv2.COLOR_RGB2BGR))
    _trace(
        "PIL decode "
        ...
        f"final_mode={rgb_pil.mode} final_size={rgb_pil.size}"
    )
    _trace(f"Decoded RGB shape={rgb_image.shape} dtype={rgb_image.dtype}")
    _trace(f"Prepared BGR shape={bgr_image.shape} dtype={bgr_image.dtype}")
...
def detect_face_and_extract_skin(image_bytes: bytes) -> dict[str, object]:
    ...
    _trace(
        "Thresholds "
        f"candidate_min_detection_confidence={CANDIDATE_MIN_DETECTION_CONFIDENCE:.2f} "
        f"min_clear_face_confidence={MIN_FACE_CONFIDENCE:.2f} "
        f"min_box_side_ratio={MIN_FACE_BOX_SIDE_RATIO:.2f} "
        f"min_box_area_ratio={MIN_FACE_BOX_AREA_RATIO:.2f}"
    )
