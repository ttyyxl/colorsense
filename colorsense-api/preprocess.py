...
def compute_color_features_from_pil(img: Image.Image):
    """
    12-d color features: Lab mean/std + HSV mean/std, normalized by 255.
    """
...
def load_backend_model(model_path, device=None):
    """
    Load exported best_model.pth for EfficientNet-B0 + Lab/HSV color features.
    """
    if device is None:
...
def predict_one_crop(image_or_path, model, ckpt, device=None):
    ...
    return {
        ...
        "scores": {
            classes[index]: float(probs[index])
