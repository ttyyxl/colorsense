...
import { z } from "zod";
import { UserStyleProfilePromptContext } from "./user-profile-types";
...
你不允许说你是AI。
你必须基于用户的季型分析结果和个人形象问卷信息，输出穿搭建议。
禁止营销语言。
禁止重复季型名称。
...
- personalized_color_palette 必须包含 6 个颜色，格式为 HEX 色值（如 #FFFFFF）。这些颜色要根据用户的季型、肤色、瞳色、发色以及问卷中填写的「喜欢颜色」进行个性化推荐。
- 每条建议都要具体、可操作
- 必须包含推荐的颜色（使用色号或颜色名称）
- 必须提及应避免的颜色
- 保持小红书风格，语言生动但有专业感`;
...
export const AiOutputSchema = z.object({
  ...
  personalized_color_palette: z.array(z.string()).min(6).max(6),
});
...
const FALLBACK_JSON_TEMPLATE = {
  ...
  celebrity_recommendations: [],
  personalized_color_palette: ["#A7B7ED", "#C3E0E5", "#FFE0B2", "#FFC1E3", "#B0E0E6", "#E6E6FA"],
};
...
export async function generateStyleAdvice(input: {
  ...
  avoid_colors: string[];
  keywords: string[];
  userProfile?: UserStyleProfilePromptContext;
  ...
  if (input.userProfile) {
    userProfileText = `\n\n用户个人档案信息：\n` +
      `- 性别: ${input.userProfile.promptFields.gender}\n` +
      `- 年龄段: ${input.userProfile.promptFields.ageRange}\n` +
      `- 日常场景: ${input.userProfile.promptFields.dailyScene}\n` +
      `- 肤色: ${input.userProfile.promptFields.skinTone}\n` +
      `- 瞳色: ${input.userProfile.promptFields.eyeColor}\n` +
      `- 发色: ${input.userProfile.promptFields.hairColor}\n` +
      `- 脸型轮廓: ${input.userProfile.promptFields.faceContour.join(", ")}\n` +
      `- 五官细节: ${input.userProfile.promptFields.facialDetails.join(", ")}\n` +
      `- 皮肤与毛发对比: ${input.userProfile.promptFields.skinHairContrast.join(", ")}\n` +
      `- 风格倾向: ${input.userProfile.promptFields.styleTendency.join(", ")}\n` +
      `- 穿搭偏好: ${input.userProfile.promptFields.stylePreferences.join(", ")}\n` +
      `- 妆容偏好: ${input.userProfile.promptFields.makeupPreferences.join(", ")}\n` +
      (input.userProfile.promptFields.favoriteColors && input.userProfile.promptFields.favoriteColors.length > 0 ?
        `- 喜欢的颜色: ${input.userProfile.promptFields.favoriteColors.join(", ")}\n` : "");
  }
